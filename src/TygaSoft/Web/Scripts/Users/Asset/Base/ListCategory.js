...
var ListCategory = {
    ...
    Add: function () {
        this.Url = "../Services/AssetService.svc/SaveCategory";
        var t = $("#treeCt");
        ...
        ListCategory.Container.dialog({
            title: "新建",
            href: '../u/yCategory.html?action=add&Id=' + node.id + '',
            closed: false,
            modal: true,
            iconCls: 'icon-save',
            buttons: [{
    ...
    Edit: function () {
        ...
        ListCategory.Container.dialog({
            title: "编辑",
            href: '../u/yCategory.html?action=edit&Id=' + node.id + '',
            closed: false,
            modal: true,
            iconCls: 'icon-save',
            buttons: [{
        ...
        if (node) {
            $.messager.confirm('温馨提醒', '确定要删除吗?', function (r) {
                if (r) {
                    $.ajax({
                        type: "POST",
                        url: "../Services/AssetService.svc/DeleteCategory",
                        contentType: "application/json; charset=utf-8",
                        data: '{"Id":"' + node.id + '"}',
                        beforeSend: function () {
                            $.messager.progress({ title: '请稍等', msg: '正在执行...' });
